...
sql1 = """
       SELECT DISTINCT a.Objbusno                        "机构编码",
                       ...
                       e.Warename                        "商品名称",
                       e.Warespec                        "商品规格"
       FROM t_Md_Notice_Accept_h a,
            t_Md_Notice_Accept_d b,
            v_Ware_Base e
       WHERE a.Billcode = 'MDACN'
         AND a.Compid = {compid}
         AND (a.Execdate >= To_Date({date1}
           , 'yyyy-MM-dd'))
         AND (a.Execdate
           < To_Date({date2}
           , 'yyyy-MM-dd'))
         ...
         AND NOT EXISTS
           (SELECT 1
           FROM t_Md_Notice_Accept_h c
           , t_Md_Notice_Accept_d d
...
if compid == "2209":
    ...
    # df1 = pd.read_sql_query(sql1, engine)
    df1 = pd.read_sql_query(sql1youhua, engine)
